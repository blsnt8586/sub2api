...
func (_q *AccountQuery) QueryProvider() *Sub2APIProviderQuery {
...
func (_q *AccountQuery) WithProvider(opts ...func(*Sub2APIProviderQuery)) *AccountQuery {
	query := (&Sub2APIProviderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) loadProvider(ctx context.Context, query *Sub2APIProviderQuery, nodes []*Account, init func(*Account), assign func(*Account, *Sub2APIProvider)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Account)
	for i := range nodes {
