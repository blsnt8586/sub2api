...
func (Sub2APIProviderProbeTarget) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Bool("adaptive_interval_enabled").Default(true),
		field.Int("healthy_interval_seconds").Default(120).Range(30, 86400),
		field.Int("healthy_interval_threshold").Default(2).Range(1, 100),
		field.Int("stable_healthy_interval_seconds").Default(300).Range(30, 86400),
		field.Int("stable_healthy_threshold").Default(6).Range(1, 100),
		field.Int("consecutive_healthy").Default(0).Min(0),
		...
		field.Int("degraded_optimize_threshold").Default(3).Range(1, 20),
		field.Bool("cost_optimize_enabled").Default(true),
		field.Int("cost_optimize_interval_seconds").Default(21600).Range(1800, 86400),
		field.Int("cost_optimize_healthy_threshold").Default(6).Range(1, 20),
		field.Time("last_cost_optimize_at").Optional().Nillable(),
