...
// JimengVideoCreation 处理即梦视频任务创建：POST /v1/videos。
func (h *OpenAIGatewayHandler) JimengVideoCreation(c *gin.Context) {
	h.handleJimengVideo(c, service.JimengVideoEndpointCreate, "")
}

// JimengVideoStatus 处理即梦视频任务状态查询：GET /v1/videos/{task_id}。
...
// JimengVideoContent 处理即梦视频内容下载：GET /v1/videos/{task_id}/content。
func (h *OpenAIGatewayHandler) JimengVideoContent(c *gin.Context) {
...
func (h *OpenAIGatewayHandler) handleJimengVideo(c *gin.Context, endpoint service.JimengVideoEndpoint, taskID string) {
	...
	isCreate := endpoint == service.JimengVideoEndpointCreate
	...
	h.runJimengVideoForwardLoop(c, endpoint, taskID, body, contentType, requestModel, apiKey, subject, subscription, &streamStarted, reqLog)
}

func (h *OpenAIGatewayHandler) runJimengVideoForwardLoop(
	c *gin.Context,
	endpoint service.JimengVideoEndpoint,
	taskID string,
	body []byte,
	contentType string,
	requestModel string,
	...
	if isCreate {
		...
		sessionHash = service.JimengVideoTaskSessionHash(taskID)
	}
	...
	for {
		...
		if err != nil {
			...
			if len(failedAccountIDs) == 0 {
				cls := classifyNoAccountErrorFromGin(c, h.gatewayService, apiKey, requestModel, requestModel, service.PlatformJimeng)
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			...
			return h.gatewayService.ForwardJimengVideo(requestCtx, c, account, endpoint, taskID, body, contentType)
		}()
		...
		if isCreate && strings.TrimSpace(result.ResponseID) != "" {
			if err := h.gatewayService.BindJimengVideoTaskAccount(requestCtx, apiKey.GroupID, result.ResponseID, account.ID); err != nil {
		...
		if isCreate && strings.TrimSpace(requestModel) != "" {
			recordJimengVideoUsage(c, h, reqLog, apiKey, subject, subscription, account, result, requestModel, body, taskID)
		}
