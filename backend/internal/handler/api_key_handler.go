...
// ListSmartGroupSwitchLogs returns successful automatic route changes from the
// last three days for an API key.  The API-key service/repository applies the
// same ownership constraint in SQL, so this endpoint cannot be used to inspect
// another user's key even if an ID is guessed.
// GET /api/v1/keys/:id/smart-group-logs
func (h *APIKeyHandler) ListSmartGroupSwitchLogs(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	logs, err := h.apiKeyService.ListSmartGroupSwitchLogs(c.Request.Context(), subject.UserID, keyID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *APIKeyHandler) Create(c *gin.Context) {
	...
	svcReq := service.CreateAPIKeyRequest{
		...
		SmartGroupFailureThreshold:        req.SmartGroupFailureThreshold,
		SmartGroupRecoveryIntervalSeconds: req.SmartGroupRecoveryIntervalSeconds,
	}
	if req.Quota != nil {
