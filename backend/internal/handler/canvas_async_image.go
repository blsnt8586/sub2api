...
// handleCanvasAsyncImage 是异步图像三端点的公共前置流程。
// 结构与 handleCanvasAudio / handleCanvasVideo 对齐：
// 鉴权 → 读 body → 计费资格检查（仅创建） → 选号 + failover 循环 → 转发。
func (h *OpenAIGatewayHandler) handleCanvasAsyncImage(c *gin.Context, endpoint service.CanvasAsyncImageEndpoint, taskID string) {
	streamStarted := false
	...
	isCreate := endpoint == service.CanvasAsyncImageCreate

	reqLog := requestLogger(
		c,
		"handler.openai_gateway.canvas_async_image",
	...
	if isCreate {
		if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), apiKey.User, apiKey, apiKey.Group, subscription, service.QuotaPlatform(c.Request.Context(), apiKey)); err != nil {
			reqLog.Info("canvas_async_image.billing_eligibility_check_failed", zap.Error(err))
	...
	h.runCanvasAsyncImageForwardLoop(c, endpoint, taskID, body, contentType, requestModel, apiKey, subject, subscription, &streamStarted, reqLog)
}

// runCanvasAsyncImageForwardLoop 走与音频/视频一致的调度 + failover 循环。
//
// 选号必须传 service.PlatformCanvas：通用的 SelectAccountForModelWithExclusions
// 内部硬编码 PlatformOpenAI，会导致 avi2api 账号一个都命中不了。
func (h *OpenAIGatewayHandler) runCanvasAsyncImageForwardLoop(
	c *gin.Context,
	endpoint service.CanvasAsyncImageEndpoint,
	taskID string,
	body []byte,
	contentType string,
	requestModel string,
	...
	if isCreate {
		...
		sessionHash = service.CanvasAsyncImageTaskSessionHash(taskID)
	}
	...
	for {
		...
		if err != nil {
			reqLog.Warn("canvas_async_image.account_select_failed",
				zap.Error(err),
		...
		reqLog.Debug("canvas_async_image.account_schedule_decision",
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			...
			return h.gatewayService.ForwardCanvasAsyncImage(requestCtx, c, account, endpoint, taskID, body, contentType)
		}()
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				switchCount++
				reqLog.Warn("canvas_async_image.upstream_failover_switching",
			...
			reqLog.Warn("canvas_async_image.forward_failed",
				zap.Int64("account_id", account.ID),
				zap.Error(err),
			)
			return
		...
		if isCreate && strings.TrimSpace(result.ResponseID) != "" {
			if err := h.gatewayService.BindCanvasAsyncImageTaskAccount(requestCtx, apiKey.GroupID, result.ResponseID, account.ID); err != nil {
				reqLog.Warn("canvas_async_image.bind_task_account_failed",
					zap.Int64("account_id", account.ID),
