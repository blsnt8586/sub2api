...
// JimengImages 处理即梦平台（Leonardo vendor）的图像生成/编辑请求。
// 对外走 OpenAI Images 兼容协议：
//
//	POST /v1/images/generations
//	POST /v1/images/edits
//
// 原生即梦无图像接口，仅 Leonardo vendor 账号支持；选中原生即梦账号时
// ForwardJimengImages 会返回明确错误。
func (h *OpenAIGatewayHandler) JimengImages(c *gin.Context) {
	endpoint := service.LeonardoImageEndpointGenerations
	if strings.Contains(c.Request.URL.Path, "/images/edits") {
		endpoint = service.LeonardoImageEndpointEdits
	}
	h.handleJimengImages(c, endpoint)
}

func (h *OpenAIGatewayHandler) handleJimengImages(c *gin.Context, endpoint service.LeonardoImageEndpoint) {
	streamStarted := false
	...
	requestModel := service.ExtractLeonardoImageModel(contentType, body)
	...
	h.runJimengImagesForwardLoop(c, endpoint, body, contentType, requestModel, apiKey, subject, subscription, &streamStarted, reqLog)
}

func (h *OpenAIGatewayHandler) runJimengImagesForwardLoop(
	c *gin.Context,
	endpoint service.LeonardoImageEndpoint,
	body []byte,
	contentType string,
	requestModel string,
	...
	for {
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			...
			return h.gatewayService.ForwardJimengImages(requestCtx, c, account, endpoint, body, contentType)
		}()
