...
func RegisterGatewayRoutes(
	...
	cfg *config.Config,
	smartGroupServices ...*service.APIKeySmartGroupService,
) {
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		// 视频路由（Grok、即梦 jimeng 等）由 registerVideoRoutes 统一管理（gateway_video.go），
		// 含上游按类型细分的 /videos/{generations,edits,extensions}/:request_id 及 catch-all
		// /videos/:request_id——当前 Gin 版本允许同层静态段与通配段并存，无需二选一。
