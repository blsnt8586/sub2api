...
// api_key 类型的 Grok 账号走独立的第三方端点转发路径（forwardGrokResponsesViaAPIKey）：
//   - 转发到 credentials 里配置的第三方 base_url，而非官方 xAI 域名；
//   - 用 credentials.api_key 作为 Bearer，不走 OAuth token provider；
//   - 不写 OAuth 订阅专属的配额快照。
func TestForwardGrokResponsesViaAPIKeyUsesThirdPartyBaseURLAndAPIKey(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := `{"id":"resp_grok_apikey","model":"grok-4.3","usage":{"input_tokens":4,"output_tokens":6}}`
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header: http.Header{
			"Content-Type":   []string{"application/json"},
	...
	result, err := svc.forwardGrokResponsesViaAPIKey(context.Background(), c, account, body, "grok", false, time.Now())
	require.NoError(t, err)
	// 用 credentials.api_key 作为 Bearer，而非 OAuth access_token。
	require.Equal(t, "Bearer third-party-key", upstream.lastReq.Header.Get("Authorization"))
	// 转发到第三方 base_url，而非官方 xAI 端点。
	require.Equal(t, "https://third-party.example.com/v1/responses", upstream.lastReq.URL.String())
	require.Equal(t, "grok-4.5", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Equal(t, "resp_grok_apikey", result.ResponseID)
	require.Equal(t, 4, result.Usage.InputTokens)
	require.Equal(t, 6, result.Usage.OutputTokens)
	require.Equal(t, http.StatusOK, recorder.Code)
	// api_key 路径不写 OAuth 订阅专属的配额快照。
	require.Nil(t, repo.updates[71][grokQuotaSnapshotExtraKey])
}

// OAuth 类型的账号不应进入 api_key 转发路径。
func TestForwardGrokResponsesViaAPIKeyRejectsOAuthAccount(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	...
	_, err := svc.forwardGrokResponsesViaAPIKey(context.Background(), c, account, []byte(`{"model":"grok"}`), "grok", false, time.Now())
	require.Error(t, err)
}
