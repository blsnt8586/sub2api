...
func normalizeOpenAICompatiblePlatform(platform string) string {
	...
	// [CUSTOM] canvas 走 OpenAI 兼容调度链路，但账号在库中以 platform=canvas 独立存储
	// （见 Account.IsOpenAICompatible 同样把 canvas 列为兼容平台）。此处必须保留原值，
	// 否则 listSchedulableAccounts 会按 platform=openai 去查，canvas 账号一个都命中不了，
