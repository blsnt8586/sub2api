...
func sub2APIProbeGroupsExhaustedPredicate() dbpredicate.Account {
	return dbpredicate.Account(func(s *entsql.Selector) {
		// Missing flags are false, including under NOT for the active filter.
		s.Where(entsql.ExprP("COALESCE(" + s.C(dbaccount.FieldExtra) + "->'sub2api_probe_groups_exhausted' = 'true'::jsonb, FALSE)"))
	})
}
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	if len(updates.Extra) > 0 || len(ollamaGroupIdentityChanges) > 0 || len(opencodeGroupIdentityChanges) > 0 || ollamaProxyIdentityChanged != "" || updates.EnsureCodexFingerprintSeed || updates.ReleaseProbeOwnershipOnNormalStop {
