  ...
  <AppLayout>
    ...
    >
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <fieldset v-if="!showEditModal && !formData.smart_group_enabled" data-tour="key-form-provider">
...
const onFixedGroupChange = (value: string | number | boolean | null) => {
	...
	formData.value.platform = groupOptions.value.find(option => option.value === formData.value.group_id)?.platform ?? null
}
...
watch([showCreateModal, createProviderOptions], ([isOpen, providers], [wasOpen]) => {
  if (!isOpen || formData.value.smart_group_enabled) return
...
const handleSubmit = async () => {
	if (formData.value.smart_group_enabled && formData.value.platform === null) {
