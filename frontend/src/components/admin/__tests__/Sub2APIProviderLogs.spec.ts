...
describe('Sub2APIProviderLogs', () => {
  ...
  it('uses the final successful candidate as the outcome after an earlier rollback', () => {
    ...
    expect(wrapper.text()).toContain('admin.sub2apiProviders.health.logs.optimization.events.rollbackForComparison')
    expect(wrapper.text()).toContain('Available group ×0.08')
    expect(wrapper.text()).toContain('admin.sub2apiProviders.health.logs.optimization.events.selected')
  })
})
